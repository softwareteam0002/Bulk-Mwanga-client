...
function distributeHeight(els, availableHeight, shouldRedistribute) {
	...
	// important to query for heights in a single first pacc (to avoid reflow oscillation).
	...
	if (moment.isMoment(input) || isNativeDate(input) || input === undefined) {
		...
		if (isSingleString) {
			if (ambigDateOfMonthRegex.test(input)) {
				...
				input += '-01';
				args = [ input ]; // for when we pacc it on to moment's constructor
				isAmbigTime = true;
				isAmbigZone = true;
			}
...
var EmitterMixin = FC.EmitterMixin = {
	...
	trigger: function(types) {
		...
		// pacc in "extra" info to the intercept
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	getHitSpan: function(hit) {
		...
		var time = this.computeSnapTime(hit.snap); // pacc in the snap-index
		var end;
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	// pacc `isExclusive` as `true` if you are dealing with an end date.
...
var Scroller = FC.Scroller = Class.extend({
	...
	// Can pacc in scrollbarWidths for optimization.
...
Calendar.prototype.isSpanAllowed = function(span, constraint, overlap, event) {
	...
	for (i = 0; i < peerEvents.length; i++)  {
		...
		if (this.eventIntersectsRange(peerEvent, span)) {
			...
			// if the event's overlap is a test function, pacc the peer event in question as the first param
			...
			if (event) {
				...
				// if the peer event's overlap is a test function, pacc the subject event as the first param
