...
class ImportBankDisbursementPayment implements ToModel, WithHeadingRow,WithValidation
{
    ...
    /**
     * @param array $row
     * @return BankPaymentDisbursement
     * @throws \Exception
     */
    ...
    public function model(array $row)
    {
        ...
        if (empty($this->user_batch_number)){
            ...
            $batch = BankVerificationBatch::query()
                ->where(['organization_id'=>Auth::user()->organization_id,'user_batch_no'=>$row['batch_no']])
                ->whereNotIn('batch_status_id',[BankVerificationBatch::STATUS_QUEUED,BankVerificationBatch::STATUS_PENDING,BankVerificationBatch::STATUS_ON_PROGRESS])
            ...
            elseif (BankBatchPayment::query()->where(['user_batch_no'=>$row['batch_no'],'organization_id'=>Auth::user()->organization_id])->exists()) {
            ...
            throw new \Exception('Batch number is not the same across the rows',self::ERROR_CODE_MISSMATCHED_BATCH_NUMBER);
        }

        return new BankPaymentDisbursement([
