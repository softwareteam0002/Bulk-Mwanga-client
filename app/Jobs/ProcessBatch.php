...
class ProcessBatch implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try{
            $batch_processing = BatchProcessing::query()->find($this->batch_processing_id);
            $batch = ($this->operation=='disburse'? BatchPayment::query():Batch::query())
    ...
    private function internalHandle($batch, $batch_processing){
        ...
        try{
            ...
            $batch_processing = BatchProcessing::query()->lockForUpdate()->find($this->batch_processing_id);
            $batch = ($this->operation=='disburse'? BatchPayment::query():Batch::query())
            ...
            if (isset($status) && $status == PaymentController::PROCESSING_STATUS_PARTIAL_FAILURE){
                ...
                $batch->update(['batch_status_id'=>Batch::STATUS_FAILED,'status_description'=>'Some entries failed']);
                $batch_processing->update([
                    'status'=>BatchProcessing::STATUS_PROCESSED,
                    'result'=>BatchProcessing::RESULT_FAILED]);
            }else{
    ...
    private function doProcessing(){
        if ($this->operation=='verify'){
            return DisbursementApiHelper::doBatchVerification($this->batch_processing_id);
        }elseif ($this->operation=='disburse'){
            return DisbursementApiHelper::doBatchPayment($this->batch_processing_id);
        }
        return null;
    }
