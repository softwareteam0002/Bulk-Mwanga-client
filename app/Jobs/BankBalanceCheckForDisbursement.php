...
class BankBalanceCheckForDisbursement implements ShouldQueue
{
    ...
    {
        $batch = BankBatchPayment::query()
            ->where(['id' => $this->batch_id])
            ->first();
        $organization_id = $batch->organization_id;

        ['status'=>$status,'message'=>$message]=BankDisbursementApiHelper::doBalanceCheck($organization_id,$this->initiatorId);
        if ($status=='failed'){
            ...
            Queue::later(Carbon::now()->addMinutes(15), new BankMonitorOpeningBalanceResult($this->batch_id));
