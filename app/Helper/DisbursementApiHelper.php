...
use App\Jobs\BalanceCheckForDisbursement;
...
class DisbursementApiHelper
{
    ...
    public static function getVerificationStatus($batch_no, $percentage_only = false, $extended_status = false)
    {
        ...
        $extended = [];
        $processed_q = Disbursement::query()
            ->where(['batch_no' => $batch->batch_no])
            ->whereIn('payment_status', [Disbursement::STATUS_PAID, Disbursement::STATUS_ERROR]);
        $processed = $processed_q->count();
        $pending_q = Disbursement::query()
            ->where(['batch_no' => $batch->batch_no])
            ->where('payment_status', '=', Disbursement::STATUS_NOT_PAID);
        $pending = $pending_q->count();
        $failed_q = Disbursement::query()
            ->where(['batch_no' => $batch->batch_no])
            ->where(['payment_status' => Disbursement::STATUS_ERROR]);
        $failed = $failed_q->count();

        $successful_q = Disbursement::query()
            ->where(['batch_no' => $batch->batch_no])
            ->where(['payment_status' => Disbursement::STATUS_PAID]);
    ...
    {
        $batch_payment = BatchPayment::query()->where(['batch_no' => $batch_no])->first();
        if (empty($batch_payment)) {
            return null;
        }
        $extended = [];
        $processed_q = DisbursementPayment::query()
            ->where(['batch_no' => $batch_payment->batch_no])
            ->whereIn('payment_status', [DisbursementPayment::STATUS_PAID, DisbursementPayment::STATUS_SENT, DisbursementPayment::STATUS_ERROR]);
        $processed = $processed_q->count();
        $pending_q = DisbursementPayment::query()
            ->where(['batch_no' => $batch_payment->batch_no])
            ->where('payment_status', '=', DisbursementPayment::STATUS_NOT_PAID);
        $pending = $pending_q->count();
        $failed_q = DisbursementPayment::query()
            ->where(['batch_no' => $batch_payment->batch_no])
            ->where(['payment_status' => DisbursementPayment::STATUS_ERROR]);
        $failed = $failed_q->count();

        $successful_q = DisbursementPayment::query()
            ->where(['batch_no' => $batch_payment->batch_no])
    ...
    {
        return (new PaymentController())->batchProcessor($batch_processing_id, 'verify');
    }
    ...
    {
        return (new PaymentController())->batchProcessor($batch_processing_id, 'disburse');
    }
    ...
    public static function queueBatchPayment($batch_id)
    {
        ...
        return self::queueBatchOperation($batch_id, 'disburse');
    }
    ...
    {
        $batch = ($operation == 'verify' ? Batch::query() : BatchPayment::query())
            ->where(['id' => $batch_id])
            ->first();
        ...
        if ($operation == 'verify') {
            $batch_processing = BatchProcessing::addQueuedBatch($operation, $batch, Auth::id());
            ...
            if (strtotime($batch->schedule_at) - time() > 0) {
                ...
                BatchProcessing::addQueuedBatch($operation, $batch, Auth::id());
                Queue::later(0, new BalanceCheckForDisbursement($batch_id, Auth::id()));
            }
        }
    ...
    {
        return (new PaymentController())->checkBalance($organization_id, $initiator);
    }
    ...
    {
        $entry = Disbursement::query()->where(['id' => $entry_id])->first();

        if (empty($entry)) {
            throw new Exception("Disbursement entry with an id of $entry_id is not found");
        }

        return (new PaymentController())->customerNameSearch($entry);
    }

    public static function testPayment($entry_id)
    {
        $entry = DisbursementPayment::query()->where(['id' => $entry_id])->first();
        if (empty($entry)) {
            throw new Exception("Disbursement entry with an id of $entry_id is not found");
        }

        $batch = BatchPayment::query()->where(['batch_no' => $entry->batch_no])->first();
        ...
        return (new PaymentController())->disbursement($organization, $entry, BatchProcessing::query()->first(),$batch->with_withdrawal_fee == 'YES');
    }
}
