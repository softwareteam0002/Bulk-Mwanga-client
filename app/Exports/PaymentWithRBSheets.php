...
class PaymentWithRBSheets implements FromView,WithTitle,WithEvents
{
    private  $startDate;
    private  $endDate;
    private  $batch;
    ...
    public function query()
    {
        ...
        /**
         * Changed from eloquent to query builder to improve performance (disbursement_payments)
         * @auth_backup baraka machumu
         */
        $data  =  DB::table('disbursement_payments as dp');
...
//            ->whereBetween('bv.created_at',[$this->startDate.' 00:00:01',$this->endDate.' 23:59:59'])
                ->where('bv.created_at','>=',$this->startDate.' 00:00:01')
            ->where('bv.created_at','<=',$this->endDate.' 23:59:59')
            //->where(['tx.status'=>'SUCCESS'])
            ->select('bv.user_batch_no','bv.id','organization_id',
                'dp.first_name',
                'dp.last_name',
                'dp.phone_number',
...
//                DB::raw('(SELECT mpesa_receipt FROM tx_disbursement WHERE entry_id=dp.id AND status="SUCCESS" LIMIT 1) as mpesa_receipt'),
        ...
        $orgName = "";
        $balance = 0;
        $handlers  =  DB::table('batch_payments')
...
//        $bp = DB::table('batch_payments')->select('id','batch_no','created_at')->where(['batch_no'=>$this->batch])->first();
            ...
            $ob_balance = DB::table('disbursement_opening_balances as dop')
                ...
                ->where(['batch_id'=>$disbursements[0]->id])
                ->where('transaction_type','!=','bank')->first();
            $balance = $ob_balance->available_balance??0;
        }


        return view('exports.disbursements_multiple_sheets_with_rb', [
